...
#define S_FUNCTION_NAME ODE_Hand_Enviroment_Model_v03
...
static double r[NUM]  = { 0.10, 0.04, 0.04, 0.04};  // radius of links; ???????????????[m]
static double m[NUM] = {10.00, 2.00, 1.00, 1.00};           // mass; ??????


static double x[NUM]= {-0.70,-0.70, -0.70,-0.70};
static double y[NUM] = {0.00};  // COG; ????????????
static double z[NUM] = {0.04, 0.75, 1.60, 2.20};

static double anchor_x[NUM]  = {-0.70,-0.70, -0.70,-0.70};
static double anchor_y[NUM] = {0.00}; // anchor point; ????????????
                          ...
                          const real_T *rWork)
{
		...
		/* reading inputs */
        dJointAddUniversalTorques(joint[1],rWork[0],rWork[1]);
        dJointAddHingeTorque(joint[2],rWork[2]);
        dJointAddHingeTorque(joint[3],rWork[2]);
//         for(i = 0; i < NUM-1; i++)
//         {
...
//         }
		...
		/* writing outputs */
        q[0]   =  dJointGetUniversalAngle1(joint[1]);
		dq[0]  = dJointGetUniversalAngle1Rate(joint[1]);
		ddq[0] = (dq[0] - dq_old[0])/time_step;
		dq_old[0] = dq[0];
		
        q[1]  = dJointGetUniversalAngle2(joint[1]);
		dq[1] = dJointGetUniversalAngle2Rate(joint[1]);
		ddq[1] = (dq[1] - dq_old[1])/time_step;
		dq_old[1] = dq[1];
        
        q[2]  = dJointGetHingeAngle(joint[2]);
		dq[2] = dJointGetHingeAngleRate(joint[2]);
