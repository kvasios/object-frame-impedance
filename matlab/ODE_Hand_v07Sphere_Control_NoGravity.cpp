...
#define S_FUNCTION_NAME ODE_Hand_v07Sphere_Control_NoGravity
...
static double surface_normal[3] ={0, 0, 0};
...
{
    switch (cmd) {
    case 'l': sphere_eq_force= 0.0; // When the j key is pushed, THETA [ angle of 1 ] increases 0.05 [ rad ]
...
// 		dBodyAddForce(sphere,0,0,sphere_eq_force);
        ...
        dBodyAddForceAtPos(sphere,rWork[19],rWork[20],rWork[21],rWork[16],rWork[17],rWork[18]);
        dBodyAddForceAtPos(sphere,rWork[25],rWork[26],rWork[27],rWork[22],rWork[23],rWork[24]);
		/* reading inputs */
...
//         sphere_position=dBodyGetPosition(sphere);
        ...
        for(int i=0; i<3;++i)
            ddq[i]=dBodyGetPosition(sphere)[i] ;
