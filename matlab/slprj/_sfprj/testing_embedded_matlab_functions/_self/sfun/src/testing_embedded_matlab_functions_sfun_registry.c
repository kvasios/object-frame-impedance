...
{
  extern unsigned int
    sf_testing_embedded_matlab_functions_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_testing_embedded_matlab_functions_process_check_sum_call(nlhs,plhs,nrhs,
       prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_testing_embedded_matlab_functions_autoinheritance_info(
    int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_testing_embedded_matlab_functions_autoinheritance_info(nlhs,plhs,nrhs,
       prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_testing_embedded_matlab_functions_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "testing_embedded_matlab_functions") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_testing_embedded_matlab_functions_get_eml_resolved_functions_info
      (nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_testing_embedded_matlab_functions_method_dispatcher
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"testing_embedded_matlab_functions")) {
    return(sf_testing_embedded_matlab_functions_method_dispatcher(simstructPtr,
...
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    testing_embedded_matlab_functions_terminator();
    sf_debug_terminate();
  }
...
extern void testing_embedded_matlab_functions_register_exported_symbols
  (SimStruct* S);
extern void testing_embedded_matlab_functions_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  testing_embedded_matlab_functions_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      testing_embedded_matlab_functions_debug_initialize();
    }

    testing_embedded_matlab_functions_initializer();
  }
